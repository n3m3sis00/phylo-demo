...
import {parseMSASeq, colorScheme } from 'phylo-react'

export function calcWidth(stri) {
    if(stri.length < 1000) return stri.length * 16 + 20
    return stri.length * 6 + 20
}

export default function MSATest(props) {
  ...
  useEffect(() => {
    ...
    const svg = d3
      .select('#metadata')
      .attr('width', dataToShow.length > 0 ? calcWidth(seqMap.get(dataToShow[0].name)) : 1000) //childLoc.length > 0 ? calcWidth(seq.get(childLoc[0].name)) : 0
